...
export class PropertyResponse implements Required<IpropertyDTO>{
    ...
    @ApiProperty({required:true, description:'количество монет которое конвертируем. Необязателен, По умолчанию 1',default:1})
    amount: number;

    @ApiProperty({required:true, description:'Результат конвертации',default:1})
    result:string
